 ...
 *
 * Each authenticated user gets their own MCP connection using their OAuth tokens.
 * Connections are cached and reused across requests.
 */
import { Client } from '@modelcontextprotocol/sdk/client/index.js';
import { StreamableHTTPClientTransport } from '@modelcontextprotocol/sdk/client/streamableHttp.js';
import Anthropic from '@anthropic-ai/sdk';
import { getUserAuth, updateUserTokens } from '../auth/store.js';
...
// Max age for cached connections (30 min)
const MAX_CONNECTION_AGE_MS = 30 * 60 * 1000;

/**
 * Get or create an MCP client for a specific user.
 * Returns their available tools, or null if not authed.
 */
async function getUserMCPClient(
  ...
  try {
    ...
    if (statusCode === 401 || message.includes('Unauthorized')) {
      ...
      const refreshed = await refreshAccessToken(phoneNumber, baseUrl);
      if (refreshed) {
        // Retry with new token
        return getUserMCPClient(phoneNumber, baseUrl);
      }
    }
...
/**
 * Get Granola tools available for a specific user.
 */
...
/**
 * Call a Granola MCP tool for a specific user.
 */
export async function callGranolaToolForUser(
  ...
  const result = await getUserMCPClient(phoneNumber, baseUrl);
  if (!result) {
    return 'Error: Not connected to Granola. Please link your account first.';
  }
...
/**
 * Check if a tool name is a Granola MCP tool.
 */
...
/**
 * Disconnect a user's MCP client (e.g., on logout).
 */
export async function disconnectUser(phoneNumber: string): Promise<void> {
  ...
  if (cached) {
    try { await cached.client.close(); } catch {}
    userClients.delete(phoneNumber);
    console.log(`[granola] Disconnected ${phoneNumber}`);
  }
}
